...
export async function list({ requestId, organizationId }: BaseService, options: BlockedTimeListOptions) {
    ...
    try {
        ...
        throw new ApiError({
            ...
            message: `Failed to list blocked times: ${error instanceof Error ? error.message : 'Unknown error'}`,
        });
    }
}
...
export async function getById({ requestId, organizationId }: BaseService, id: string) {
    ...
    try {
        ...
        throw new ApiError({
            code: 'INTERNAL_SERVER_ERROR',
            message: `Failed to get blocked time: ${error instanceof Error ? error.message : 'Unknown error'}`,
        });
    }
}
